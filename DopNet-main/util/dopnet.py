...
def calc_atom_feats(elem_feats, comp, max_dops):
    ...
    if len(host_feats) == 0:
        # host_feats为空时，用0向量代替 std, min, max
        dim = elem_feats.shape[1]
        host_feat = numpy.hstack([
            w_sum_vec,
            numpy.zeros(dim),  # std
            numpy.zeros(dim),  # min
            numpy.zeros(dim)   # max
        ...
        host_feats_arr = numpy.vstack(host_feats)
        host_feat = numpy.hstack([
            w_sum_vec,
            numpy.std(host_feats_arr, axis=0),
            numpy.min(host_feats_arr, axis=0),
            numpy.max(host_feats_arr, axis=0)
        ])
